#!/usr/bin/env python3
"""Benchmark shebangsy against reference runners and publish results.

**Goal:** Measure end-to-end script startup/runtime for each asset under
``scripts/bench-assets/`` (shebangsy scripts, plain ``bin`` baselines, and other
runners), in milliseconds, on the developer’s machine.

**Why:** You want repeatable numbers to compare implementations and to notice when
shebangsy gets slower or faster relative to a compiled ``bin`` for the same
language—i.e. regression signal, not a CI gate.

**How:** Build ``dist/shebangsy``, preflight every bench command, then run
``hyperfine`` on the main assets listed per language in ``BENCH_LANGUAGE_META`` (``apps``;
``HYPERFINE_BENCHMARK_ROUNDS`` rounds × warmup + ``HYPERFINE_RUNS_PER_COMMAND`` measured runs,
``--shell=none``). Entries in ``slow_apps`` run in **one** extra hyperfine pass with
``HYPERFINE_SLOW_RUNS`` runs, then merge into the same result shape.
Means are grouped by
language and appended as one JSON object per run to ``benches.jsonl`` (with CPU
and unix time). After each successful run, ``benches-report.md`` is regenerated
(with Mermaid charts for shebangsy-only overhead vs ``bin`` over time and for
latest absolute and overhead snapshots), and the ``### Results`` block in
``README.md`` is updated to the same mean absolute-time bar chart as the report.
"""
...
def _bench_language_meta(lang: str) -> dict[str, Any]:
  """Merge ``BENCH_LANGUAGE_META_DEFAULTS`` with the row for ``lang``, if any."""
...
def _normalize_hyperfine_command(cmd: str) -> str:
  """Stable key for matching hyperfine ``command`` strings to aliases."""
  stripped = cmd.strip()
  if not stripped:
    return ""
  try:
...
def _hyperfine_command_for_grouping(
  ...
  """Map hyperfine-reported command to grouping path for ``_bench_asset_parts``."""
  table = COMMAND_ALIASES if aliases is None else aliases
  norm = _normalize_hyperfine_command(cmd)
  return table.get(norm, cmd)


def _merge_slow_interpreter_aliases() -> dict[str, str]:
  """Alias ``interpreter`` invocations for ``slow_apps`` to the same ``<lang>/bin`` path as main."""
...
def _collect_slow_benchmark_commands(root_dir: Path) -> list[str]:
  """``slow_apps`` under ``bench-assets/<lang>/``, plus optional interpreter baselines."""
  cmds: list[str] = []
...
def _bench_asset_parts(command_name: str) -> tuple[str, str] | None:
  """If command is under bench-assets, return (language_dir, key_under_that_dir)."""
...
def _cpu_label() -> str:
  """Return a short CPU or machine identifier for benchmark metadata."""
...
def _flatten_results(results: dict[str, dict[str, float]]) -> dict[str, float]:
  """Flatten nested language -> asset -> ms into ``lang/asset`` keys."""
  flat: dict[str, float] = {}
...
def _mermaid_axis_label(key: str) -> str:
  """Make a short x-axis token safe for Mermaid ``[a, b, c]`` lists."""
...
def _plot_color_palette_csv(count: int) -> str:
  """Comma-separated hex list for Mermaid ``plotColorPalette`` (series order)."""
  if count <= 0:
    return "#6b7280"
...
def _mermaid_xychart_frontmatter(
  *,
  ...
  """YAML frontmatter: light panel background and readable axis/title colors."""
  palette = (
...
def _overhead_series_keys(records: list[dict]) -> list[str]:
  """``lang/asset`` for shebangsy scripts only: non-``bin`` and ``shebangsy`` in the filename."""
  keys: set[str] = set()
  for row in records:
...
def _overhead_ms_for_run(row: dict, lang: str, asset: str) -> float | None:
  """``asset_ms - bin_ms`` for one language in a row, or ``None`` if not computable."""
  results = row.get("results")
  if not isinstance(results, dict):
    return None
...
def _mermaid_line_chart_overhead_vs_bin(records: list[dict]) -> str:
  """Line chart: per script, ``runtime_ms - bin_ms`` over time (regression vs compiled baseline)."""
  empty_chart = "\n".join(
    [
      "```mermaid",
...
def _sort_bar_items_by_language_cluster(
  ...
  """Bar order: language name ascending (A–Z), then metric ascending within each language."""
...
def _mermaid_horizontal_bar_absolute_time(
  ...
  """Single horizontal bar chart; ``items`` are ``(lang/asset, ms)``."""
...
def _mermaid_bar_latest(records: list[dict]) -> str:
  """Bar chart of flattened apps for the most recent record (bars clustered by language)."""
  empty = "\n".join(
    [
      "```mermaid",
      _mermaid_xychart_frontmatter(series_count=1),
...
def _absolute_time_mean_last_n(records: list[dict], n: int) -> list[tuple[str, float]]:
  """Mean wall-clock ms per ``lang/asset`` over the last ``n`` bench rows (or fewer)."""
  if not records or n <= 0:
    return []
  window = records[-min(n, len(records)) :]
  keys: set[str] = set()
  for row in window:
    res = row.get("results")
...
def _mermaid_bar_absolute_time_all_runs_mean(records: list[dict]) -> str:
  """Mean absolute time bar chart over every row in ``benches.jsonl``."""
  placeholder = "\n".join(
    [
      "```mermaid",
      _mermaid_xychart_frontmatter(series_count=1),
...
def _latest_shebangsy_overhead_pairs(last_results: object) -> list[tuple[str, float]]:
  """``(lang/asset, overhead_ms)`` for shebangsy assets with a ``bin`` baseline in one results dict."""
...
def _shebangsy_overhead_mean_last_n(records: list[dict], n: int) -> list[tuple[str, float]]:
  """Mean overhead (script ms − bin ms) per shebangsy asset over the last ``n`` bench rows (or fewer)."""
  if not records or n <= 0:
    return []
  window = records[-min(n, len(records)) :]
  keys: set[str] = set()
...
def _mermaid_bar_shebangsy_overhead_all_runs_mean(records: list[dict]) -> str:
  """Mean overhead bar chart over every row in ``benches.jsonl`` (shebangsy vs ``bin``)."""
  placeholder = "\n".join(
    [
      "```mermaid",
      _mermaid_xychart_frontmatter(series_count=1),
...
def _mermaid_bar_latest_shebangsy_overhead(records: list[dict]) -> str:
  """Horizontal bars: latest run, shebangsy-only overhead vs ``bin`` (same filter as the line chart)."""
  placeholder = "\n".join(
    [
      "```mermaid",
      _mermaid_xychart_frontmatter(series_count=1),
...
def _patch_readme_results_chart(readme_path: Path, chart: str) -> int:
  """Replace the first `` ```mermaid `` … `` ``` `` block after ``### Results``."""
  try:
...
def run_bench(root_dir: Path) -> int:
  ...
  for bench_cmd in benchmark_commands + slow_benchmark_commands:
    try:
      subprocess.run(
